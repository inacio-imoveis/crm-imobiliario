...
// ============ ROTA TEMPORÁRIA: limpeza de leads duplicados ============
// Criada para corrigir duplicatas geradas antes da correção do bot Ana (20/06/2026).
// Protegida por chave própria (não usa login de usuário). Remover após o uso.
const CLEANUP_KEY = "limpeza-leads-2026-temp";

function checarCleanupKey(req, res, next) {
  if (req.headers["x-cleanup-key"] !== CLEANUP_KEY)
    return res.status(401).json({ erro: "Não autorizado" });
  next();
}

// Modo dry-run: mostra os grupos de duplicatas (mesmo fone, etapa 'novo', mais de 1 registro)
// sem alterar nada
app.get("/api/admin/leads-duplicados", checarCleanupKey, async (req, res) => {
  const r = await pool.query(`
    SELECT fone, array_agg(id ORDER BY criado_em ASC) as ids,
           array_agg(nome ORDER BY criado_em ASC) as nomes,
           array_agg(imovel ORDER BY criado_em ASC) as imoveis,
           array_agg(observacoes ORDER BY criado_em ASC) as observacoes_lista,
    ...
    ORDER BY COUNT(*) DESC
  `);
  res.json({ grupos: r.rows.length, detalhe: r.rows });
});

// Mescla cada grupo de duplicatas: mantém o registro mais antigo (primeiro id), atualiza
// com o imóvel/observações mais recentes do grupo, apaga os demais (historico vai junto via CASCADE)
app.post("/api/admin/leads-duplicados/mesclar", checarCleanupKey, async (req, res) => {
  ...
  for (const grupo of grupos.rows) {
    ...
    // Pega o registro mais recente do grupo para usar como dado mais atual
    const maisRecente = await pool.query(
      "SELECT nome, imovel, observacoes FROM leads WHERE id = $1",
      [ids[ids.length - 1]]
    );
    const dados = maisRecente.rows[0];

    await pool.query(
      "UPDATE leads SET nome=$1, imovel=$2, observacoes=$3 WHERE id=$4",
      [dados.nome, dados.imovel, dados.observacoes, idPrincipal]
    );
    await pool.query(
      "INSERT INTO historico (lead_id, texto) VALUES ($1, $2)",
      [idPrincipal, `Mesclado com ${idsRemover.length} duplicata(s): ids ${idsRemover.join(", ")}`]
    );
    await pool.query("DELETE FROM leads WHERE id = ANY($1)", [idsRemover]);

    resultado.push({ fone: grupo.fone, mantido: idPrincipal, removidos: idsRemover });
  }

  res.json({ ok: true, grupos_mesclados: resultado.length, detalhe: resultado });
});
